...
int main() {
	...
	// Итераторы
	// Итераторы unordered_map являются однонаправленными итераторами - тип итераторов, который
	// поддерживает инкремент, сравнение через == и !=, но не поддерживает декремент, >/<, смещение на n и индексацию
	// У unordered_map есть 2 типа итерраторов: обычный, константный обычный.

	// Инвалидация
	// В unordered_map итераторы НЕ инвалидируются при добавлении новых элементов и удалении элементов.
