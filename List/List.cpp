...
// std::forward_list - реализация односвзяного списка.
// Вставка в начало: O(1), в конец O(n), в произвольное место через insert O(n), O(1) - если известен итератор на предыдущие элемент, т.к. тогда нужно будет
// просто поменять указетли.
// Удаление имеет такие же асимтотические сложности
// Доступ к элементу: O(n), поиск O(n), сортировка O(n log n)
...
int main()
{
	...
	// Метод sort() сортирует двусвязный список с помощью алгоритма merge sort. Соответственно, его асимптотическая сложность O(n log n).
