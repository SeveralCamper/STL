...
// Внутри std::map реализован как самобалансирующееся двоичное дерево поиска (обычно это красно-черное дерево),
...
int main()
{
	...
	// Итераторы
	// Итераторы map являются итераторами двунаправленного доступа - тип итераторов, который
	// поддерживает инкремент, декремент, сравнение через == и !=, но не поддерживает >/<, смещение на n и индексацию
	// У map есть 4 типа итерраторов: обычный, реверсный, константный обычный, константный реверсный

	// Инвалидация
	// В map итераторы НЕ инвалидируются при добавлении новых элементов и удалении элементов.
