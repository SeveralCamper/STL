...
// Если использовать std::next, то можно получить доступ к элементу по позиции, таким образом мы получи доступ до любого элемента, кроме первого и последнего за O(n)
// Поиск: (find): O(log N)
